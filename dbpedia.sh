...
for keyword in athlete film animal
do
	for number_of_loop in 1 2 3 4
	do 
		echo $keyword$number_of_loop
		mkdir -p datasets/$DATASET/$keyword$number_of_loop
		echo $keyword > datasets/$DATASET/${LABEL_NAME_FILE}
		...
		mv datasets/$DATASET/*.pt datasets/$DATASET/$keyword$number_of_loop
		echo "Running Model"
		python3 datasets/$DATASET/run.py --dataset_dir datasets/${DATASET} --keyword $keyword --number_of_loop $number_of_loop
	done
done
